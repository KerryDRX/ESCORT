...
from convs.memo_resnet import get_resnet18_imagenet as get_memo_resnet18
...
class BaseNet(nn.Module):
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        self.convnet.load_state_dict(checkpoint['model_state_dict']['convnet'])
        self.fc.load_state_dict(checkpoint['model_state_dict']['fc'])
...
class FOSTERNet(nn.Module):
    ...
    def forward(self, x):
        outputs = [convnet(x) for convnet in self.convnets]

        features = [output["features"] for output in outputs]
        features = torch.cat(features, 1)

        fmaps = [output["fmaps"][-1] for output in outputs]
        fmaps = torch.cat(fmaps, 1)

        out = self.fc(features)
        fe_logits = self.fe_fc(features[:, -self.out_dim :])["logits"]

        out.update({"fe_logits": fe_logits, "features": features, "fmaps": fmaps})
