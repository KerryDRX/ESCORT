...
class Compress:
    ...
    def _resize(self, h, w):
        k = self.params['image_resolution'] / min(h, w)
        H, W = float(k * h), float(k * w)
        while H * W > 786432:  # 512 * 512 * 3
            H /= 2; W /= 2
    ...
    def _compress(self, image_path, image):
        edge_path = to_edge_map(image_path)
        if os.path.exists(edge_path):
            edge_map = np.array(Image.open(edge_path))
        else:
            edge_map = self.apply_canny(np.array(image), self.params['low_threshold'], self.params['high_threshold'])
            self._save(edge_map, edge_path)
        return edge_map
        
    def _generate(self, edge_map, exemplar_path, seed, prompt=''):
        if os.path.exists(exemplar_path): return Image.open(exemplar_path)

        guess_mode = prompt == ''
        ddim_steps = 50 if guess_mode else 20
        scale = 4.5 if guess_mode else 9.0

        print(f'Prompt: {prompt}')
        h, w = edge_map.shape
        H, W = self._resize(h, w)
        edge_map = HWC3(cv2.resize(edge_map, (W, H), interpolation=cv2.INTER_NEAREST))
        with torch.no_grad():
    ...
    def __call__(self, image_path, seed):
        ...
        if os.path.exists(exemplar_path): return
        image = Image.open(image_path).convert('RGB')
        edge_map = self._compress(image_path, image)
        self._generate(edge_map, exemplar_path, seed, prompt=to_prompt(image_path))
