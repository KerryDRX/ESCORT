...
class DER(BaseLearner):
    ...
    def train(self):
        self._network.train()
        if len(self._multiple_gpus) > 1: self._network_module_ptr = self._network.module
        else: self._network_module_ptr = self._network
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task == 0:
            optimizer = optim.SGD(filter(lambda p: p.requires_grad, self._network.parameters()), lr=init_lr, momentum=0.9, weight_decay=init_weight_decay)
